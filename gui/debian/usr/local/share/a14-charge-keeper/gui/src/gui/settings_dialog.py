...
class SettingsDialog(QDialog):
    ...
    def _setup_window_properties(self):
        ...
        # Fix focus issues when opening from system tray
    ...
    def _setup_ui(self):
        ...
        # COMPLETELY disable mouse wheel and drag functionality
        self.threshold_spinbox.setFocusPolicy(Qt.StrongFocus)
        self.threshold_spinbox.wheelEvent = lambda event: None
        self.threshold_spinbox.mouseMoveEvent = lambda event: None
        self.threshold_spinbox.mousePressEvent = lambda event: QSpinBox.mousePressEvent(self.threshold_spinbox, event) if event.button() == Qt.LeftButton else None
        ...
        # COMPLETELY disable mouse wheel and drag functionality
        self.refresh_interval_spinbox.setFocusPolicy(Qt.StrongFocus)
        self.refresh_interval_spinbox.wheelEvent = lambda event: None
        self.refresh_interval_spinbox.mouseMoveEvent = lambda event: None
        self.refresh_interval_spinbox.mousePressEvent = lambda event: QSpinBox.mousePressEvent(self.refresh_interval_spinbox, event) if event.button() == Qt.LeftButton else None
    ...
    def showEvent(self, event: QShowEvent):
        """Handle dialog show event to fix widget state issues."""
        super().showEvent(event)
        
        # Clear focus from all widgets to prevent mouse tracking issues
        self.threshold_spinbox.clearFocus()
        self.refresh_interval_spinbox.clearFocus()
        
        # Ensure mouse tracking is disabled
        self.threshold_spinbox.setMouseTracking(False)
        self.refresh_interval_spinbox.setMouseTracking(False)
        
        # Release mouse if captured
        if self.threshold_spinbox.hasMouseTracking():
            self.threshold_spinbox.releaseMouse()
        if self.refresh_interval_spinbox.hasMouseTracking():
            self.refresh_interval_spinbox.releaseMouse()
            
        # Set focus to a safe widget (like OK button)
